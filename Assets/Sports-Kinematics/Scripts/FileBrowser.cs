...
// Contact me here for any questions: https://forum.unity3d.com/threads/runtime-file-browser.105698/
// Edited by Aiden Triffitt on 03/05/2017

namespace SportsKinematics
{
    public class FileBrowser : MonoBehaviour//FR6 - UI environment for client/test administrator.
    {
        ...
        List<string> m_filesToOpen;
        static Rect BrowserRect = new Rect(50, 50, 550, 450);

        bool m_leftCtrl, m_rightCtrl;
        ...
        private void Start()
        {
            ...
            switch (m_event)
            {
                case EventHandle.CreatePlayist:
                    m_folderPath = PlayerPrefs.GetString("CurrentUserDataPath") + "/Actions/";
                    break;
                case EventHandle.EditAction:
                    m_folderPath = PlayerPrefs.GetString("CurrentUserDataPath") + "/Actions/";
                    break;
                case EventHandle.SelectPlaylist:
                    m_folderPath = PlayerPrefs.GetString("CurrentUserDataPath") + "Playlists/";
                    break;
            }
        }
        ...
        {
            if (Directory.Exists(m_folderPath)) //Folder exists
            {
                WindowOpen = true;
                WinType = WindowType.Open;
                DragStat.Dragging = false;    // Ensure nothing is being draged.

                MainFolder = m_folderPath; // example: Application.dataPath/Saves;
            ...
            {
                if (File.Exists(m_folderPath)) // Folder is actually a file
                    Debug.LogError("I need a main directory, not a file!");
                else // Folder is not a file or folder
                    Directory.CreateDirectory(m_folderPath); //Create folder.
            }
        }
        ...
        private bool IsFileToOpenValid(string fileToOpen)
        {
            return !((fileToOpen[fileToOpen.Length - 1] == '/') || (fileToOpen[fileToOpen.Length - 1] == '\\'));
        }

        void MyOpenActionForPlaylistBuilding(string fileToOpen)//FR6 - UI environment for client/test administrator.
        {
            string[] lineSplit;
            string line;
            m_playlistBuilder = m_playlistBuilderGameObject.GetComponent<PlaylistBuilder>();

            if(IsFileToOpenValid(fileToOpen) && !fileToOpen.EndsWith(".txt"))
            {
                using (StreamReader sr = new StreamReader(fileToOpen))
                {
                    while (sr.Peek() >= 0)
                    {
                        ...
                        PlayerPrefs.SetString("ActionData", line);
                        PlayerPrefs.SetString("ActionName", lineSplit[0]);
                        PlayerPrefs.SetString("Mode", lineSplit[4]);
                        m_playlistBuilder.AddScenario();
                    }
                }
            }
            else
        ...
        void MyOpenEditor(string fileToOpen)//FR6 - UI environment for client/test administrator.
        {
            string line;
            ...
            if (IsFileToOpenValid(fileToOpen) && !fileToOpen.EndsWith(".txt"))
            {
                using (StreamReader sr = new StreamReader(fileToOpen))
                {
                    ...
                    {
                        line = sr.ReadLine();
                        PlayerPrefs.SetString("LoadedActionData", line);
                        PlayerPrefs.SetString("Action", line.Split(',')[0]);
                    }
                }
                GetComponent<UI.CheckLoad>().Interactable = true;
            }
            else
            {
                Debug.Log("Error reading file!");
        ...
        void MyOpenPlaylist(string fileToOpen)//FR6 - UI environment for client/test administrator.
        {
            if (IsFileToOpenValid(fileToOpen) && fileToOpen.EndsWith(".plist") && !fileToOpen.EndsWith(".txt"))
            {
                string[] split1 = fileToOpen.Split('\\');
                string[] split2 = split1[split1.Length - 1].Split('/');
                string[] split3 = split2[split2.Length - 1].Split('.');

                PlayerPrefs.SetString("PlaylistPath", fileToOpen);
                GameObject.Find("Simulation/SimulationCanvas/CurrentPlaylistNameLabel").GetComponent<UI.CheckPlaylistLoad>().ChangeText(split3[0]);
            }
            else
            {
                Debug.Log("Error reading file!");
        ...
        void DeleteFolders()
        {
            foreach (string FileToOpen in m_filesToOpen)
            {
                if (FileToOpen.EndsWith(".plist"))
                {
                    string filename = System.IO.Path.GetFileName(FileToOpen);
                    Directory.Delete(PlayerPrefs.GetString("CurrentUserDataPath") + "/Configurations/" + filename.Split('.')[0] + "/", true);
                    File.Delete(PlayerPrefs.GetString("CurrentUserDataPath") + "Playlists/" + filename);
                    GetAllSubDirectoriesAndFiles(MainFolder);

                    string temp1 = System.IO.Path.GetFileNameWithoutExtension(FileToOpen);
                    string temp2 = System.IO.Path.GetFileNameWithoutExtension(PlayerPrefs.GetString("PlaylistPath"));

                    if (temp1.Equals(temp2))
                    {
                        PlayerPrefs.DeleteKey("PlaylistPath");
                        GameObject.Find("Simulation/SimulationCanvas/CurrentPlaylistNameLabel").GetComponent<UI.CheckPlaylistLoad>().ChangeText("None");
                    }
                }
            }
        ...
        void DrawBrowser(int windowID)//FR6 - UI environment for client/test administrator.
        {
            ...
            if (deletingFolder) // Folder creation tool.
            {
                if ((GUI.Button(bottom, "Double click to delete")) && (m_filesToOpen.Count != 0))
                {
                    DeleteFolders();
                }
            }
            ...
            if (DragStat.Dragging && DragStat.type == DragType.WindowScaler)
            {
                ...
                if (BrowserRect.width > Screen.width - 100.0f) BrowserRect.width = Screen.width - 100.0f;
                if (BrowserRect.height > Screen.height - 100.0f) BrowserRect.height = Screen.height - 100.0f;
            }
            ...
            { 
                foreach (string FileToOpen in m_filesToOpen)
                {
                    if (File.Exists(FileToOpen) && !Directory.Exists(FileToOpen))
                    {
                        ...
                        {
                            case EventHandle.CreatePlayist:
                                MyOpenActionForPlaylistBuilding(FileToOpen);
                                break;
                            case EventHandle.EditAction:
                                MyOpenEditor(FileToOpen);
                                break;
                            case EventHandle.SelectPlaylist:
                                MyOpenPlaylist(FileToOpen);
                                break;
                        }
                    }
                    
                    else
                        if (!Directory.Exists(FileToOpen)) Debug.LogError("The File you are trying to open must have been moved, or deleted.");
                }

                m_filesToOpen.Clear();

                WindowOpen = false;
                //Prevent the parent canvas soft locking if no files selected
                switch (m_event)
                {
                    case EventHandle.CreatePlayist:
                        GameObject.Find("Simulation/PlaylistBuilderCanvas").GetComponent<CanvasGroup>().blocksRaycasts = true;
                        break;
                    case EventHandle.EditAction:
                        GameObject.Find("Editing/EditingCanvas").GetComponent<CanvasGroup>().blocksRaycasts = true;
                        break;
                    case EventHandle.SelectPlaylist:
                        GameObject.Find("Simulation/SimulationCanvas").GetComponent<CanvasGroup>().blocksRaycasts = true;
                        break;
                }
            }
            ...
            for (int i = 0; i < ScrollPositions.Length; ++i)
            {
                ...
                for (int j = 0; j < PathIndexes.Length; ++j)
                {
                    ...
                    if (GUI.Button(viewRect, PathTree[index].Name))
                    {
                        ...
                        AddFile(PathTree[index].Parent + "/" + PathTree[index].Name);
                    }
        ...
        void AddFile(string add)
        {
            if ((m_leftCtrl || m_rightCtrl) && m_event == EventHandle.CreatePlayist)
            {
                m_filesToOpen.Add(add);
            }
            else
                if (m_event != EventHandle.CreatePlayist || !(m_leftCtrl || m_rightCtrl))
                {
                    m_filesToOpen.Clear();
                    m_filesToOpen.Add(add);
                }
